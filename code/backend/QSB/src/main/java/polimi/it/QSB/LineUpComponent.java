...
public class LineUpComponent {
    ...
    public String noSenseTime(EnqueueData enqueueData) throws Exception {
        Date newHour = new Date(3600000L * 2);
        Date newMinute = new Date(60000L * 5);
        Date newMinute2 = new Date(60000L * 0);
        Date newMinute10 = new Date(60000L * 100);
        ...
        if (enqueueData.getTimeToGetToTheShop().before(newMinute2))
            return "time to get to the shop  too little";
        if (enqueueData.getTimeToGetToTheShop().after(newMinute10))
            return "time to get to the shop  too long";
        return "OK";
    }
    ...
    public Response dequeue(int ticketid) {
        ...
        try {
            ...
            /*TicketSchedulerComponent tsc = new TicketSchedulerComponent(ticketService.
                    findAllTicketsForShopAndDetach(ticketService.find(ticketid).getShop().getId()));
            List<Ticket> tickets = tsc.buildQueue();
            ticketService.mergeAllTickets(tickets);*/
        } catch (Exception e) {
            status = Response.Status.INTERNAL_SERVER_ERROR;
