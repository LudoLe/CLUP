...
--per Valid si intende che il QR code è scannerizzabile, cioè il timestamp associato è 5 minuti prima e dopo l'orario di entrata			
--per InUse si intende un biglietto è stato scannerizzato			
--per Expired si intende che il QR code non è più scannerizzabile, cioè o 5 minuti dopo l'orario d'entrata senza che sia stato registarato un TimeStamp "InUse", oppure dopo essere stato InUse			
      ...
      --A ticket is always created as Valid			
        ...
        all t:Ticket,  t2:Time |			
 		--Once a ticket is expired, it cannot change status	
                  (t.ticketStatus.t2 = Expired implies 			
			all t3: Time | gte[t3, t2] implies t.ticketStatus.t3 = Expired)
		and	
               --Once a ticket is "InUse" it cannot go back to "Valid"			
            (t.ticketStatus.t2 = InUse implies			
		all t4:Time | gte[t4, t2] implies t.ticketStatus.t4 != Valid)	
}										
...
-- i ticket generati dal totem sono necessariamente associati allo shop a cui è associato il totem			
...
--personalmente introdurrei qualcosa tipo che un utente può avere al massimo tot ticket, ma vedi te			
